...
__attribute__((weak)) bool epdiy_i2c_is_initialized(void) {
    return false;
}
...
static void epd_board_init(uint32_t epd_row_width) {
    ...
    if (!epdiy_i2c_is_initialized()) {
        ...
        if (i2c_err != ESP_OK && i2c_err != ESP_ERR_INVALID_STATE && i2c_err != ESP_FAIL) {
...
static void epd_board_set_ctrl(epd_ctrl_state_t* state, const epd_ctrl_state_t* const mask) {
    ...
    if (mask->ep_output_enable || mask->ep_mode || mask->ep_stv) {
        ...
        if (err != ESP_OK) {
            ...
            // If this fails, try once more before asserting.
